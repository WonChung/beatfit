...
export interface AppleMusicPlaylist { id: string; name: string; artworkUrl?: string; trackCount?: number; }
export interface AppleMusicTrack extends Song { id: string; isPlayable: boolean; }
export interface AppleMusicPage<T> { items: T[]; next?: string; }
...
export function toBeatFitSongs(tracks: AppleMusicTrack[]): Song[] {
  return tracks.filter((track) => track.isPlayable && track.duration_ms > 0).map(
