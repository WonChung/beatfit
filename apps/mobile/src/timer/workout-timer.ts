...
export function reconcileWorkoutTimer(
  ...
  if (!timeline[state.currentIndex]) return completedState(timeline.length);
  ...
  if (nextIndex >= timeline.length) return completedState(timeline.length);
...
function moveToInterval(
  ...
  if (!timeline[targetIndex]) return completedState(timeline.length);
...
function completedState(currentIndex: number): WorkoutTimerState {
  return {
    status: 'completed',
    currentIndex,
    intervalStartedAtMs: null,
